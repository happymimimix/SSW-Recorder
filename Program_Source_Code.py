    ...
    if win32gui.GetForegroundWindow() == MainWindowHWND:
        ...
        if Unloading:
            ...
            if Show_ControlPanel:
                if Toolbox_Open:
                    if InRange(X, Y, 325, H - 160, 441, H - 140):
                        print("  |Button: Toolbox - MTC quick fix", end='')
                        draw_rectangle(hwnd, 325, H - 160, 441, H - 140, [127, 127, 247])
                    ...
                    if InRange(X, Y, 449, H - 160, 565, H - 140):
                        print("  |Button: Toolbox - Start over", end='')
                        draw_rectangle(hwnd, 449, H - 160, 565, H - 140, [127, 127, 247])
                        mouse.wait(mouse.LEFT, mouse.UP)
                        playsound.playsound(".\\Resources\\KDE_Click.wav")
                        keyboard.press_and_release('alt')
                        keyboard.press_and_release('p')
                    ...
                    if InRange(X,Y,325, H - 135, 441, H - 115):
                        print("  |Button: Toolbox - Playback controls", end='')
                        draw_rectangle(hwnd, 325, H - 135, 441, H - 115, [127, 127, 247])
                        mouse.wait(mouse.LEFT, mouse.UP)
                        playsound.playsound(".\\Resources\\KDE_Click.wav")
                        keyboard.press_and_release('alt')
                        ...
                        keyboard.press_and_release('p')
                    if InRange(X,Y,449, H - 135, 565, H - 115):
                        print("  |Button: Toolbox - Score type settings", end='')
                        draw_rectangle(hwnd, 449, H - 135, 565, H - 115, [127, 127, 247])
                        mouse.wait(mouse.LEFT, mouse.UP)
                        playsound.playsound(".\\Resources\\KDE_Click.wav")
                        keyboard.press_and_release('alt')
                        keyboard.press_and_release('p')
                    ...
                    if InRange(X,Y,449, H - 110, 565, H - 90):
                        ...
                        keyboard.press_and_release('p')
                    if InRange(X,Y,325, H - 85, 441, H - 65):
                        print("  |Button: Toolbox - Save as *.ss10 file", end='')
                        draw_rectangle(hwnd, 325, H - 85, 441, H - 65, [127, 127, 247])
                        mouse.wait(mouse.LEFT, mouse.UP)
                        playsound.playsound(".\\Resources\\KDE_Click.wav")
                        keyboard.press_and_release('alt')
                        keyboard.press_and_release('p')
                    ...
                    if InRange(X,Y,449, H - 85, 565, H - 65):
                        print("  |Button: Toolbox - Equal width bar", end='')
                        draw_rectangle(hwnd, 449, H - 85, 565, H - 65, [127, 127, 247])
                        mouse.wait(mouse.LEFT, mouse.UP)
                        playsound.playsound(".\\Resources\\KDE_Click.wav")
                        keyboard.press_and_release('alt')
...
def Main_Program(hwnd, W, H, CX, CY):
    global Unloading
    global Show_Statistics
    if Unloading:
        draw_rectangle(hwnd, 100, H - 300, 800, H - 150, [63, 63, 63])
        draw_text(hwnd, 120, H - 280, 30, [255, 255, 255], "Are you sure you want to unload this plugin? ", 'L', 'T')
        draw_rectangle(hwnd, 650, H - 210, 750, H - 170, [191, 0, 0])
        draw_rectangle(hwnd, 500, H - 210, 600, H - 170, [0, 191, 0])
        draw_text(hwnd, 550, H - 175, 24, [255, 255, 255], "YES", 'M', 'B')
        draw_text(hwnd, 700, H - 175, 24, [255, 255, 255], "NO", 'M', 'B')
        time.sleep(1 / 128)
    else:
        global Flipping
        ...
        if Show_ControlPanel:
            if Toolbox_Open:
                draw_rectangle(hwnd, 315, H - 200, 575, H - 56, [0, 191, 255])
                draw_text(hwnd, 330, H - 190, 20, [255, 255, 0], "Toolbox", 'L', 'T')
                draw_rectangle(hwnd, 325, H - 160, 441, H - 140, [247, 247, 247])
                draw_text(hwnd, 383, H - 150, 12, [15, 15, 15], "MTC Quick Fix", 'M', 'M')
                draw_rectangle(hwnd, 449, H - 160, 565, H - 140, [247, 247, 247])
                draw_text(hwnd, 507, H - 150, 12, [15, 15, 15], "Start Over", 'M', 'M')
                draw_rectangle(hwnd, 325, H - 135, 441, H - 115, [247, 247, 247])
                draw_text(hwnd, 383, H - 125, 12, [15, 15, 15], "Playback Controls", 'M', 'M')
                ...
                draw_text(hwnd, 383, H - 75, 12, [15, 15, 15], "Save As *.ss10 File", 'M', 'M')
                draw_rectangle(hwnd, 449, H - 85, 565, H - 65, [247, 247, 247])
                draw_text(hwnd, 507, H - 75, 12, [15, 15, 15], "Equal Width Bar", 'M', 'M')
            draw_rectangle(hwnd, 300, H - 200, 15, H - 15, [255, 191, 0])
            draw_text(hwnd, 30, H - 190, 20, [0, 127, 255], "SSW Recorder Control Panel", 'L', 'T')
            draw_text(hwnd, 30, H - 156, 12, [127, 0, 0], "Auto speedhack override: ", 'L', 'T')
            draw_text(hwnd, 30, H - 138, 12, [127, 0, 0], "Recording speed: ", 'L', 'T')
            draw_text(hwnd, 30, H - 120, 12, [127, 0, 0], "Misrecognition protection level: ", 'L', 'T')
            draw_text(hwnd, 30, H - 85, 12, [127, 0, 0], "Show watermark: ", 'L', 'T')
            draw_text(hwnd, 30, H - 65, 12, [127, 0, 0], "Show statistics: ", 'L', 'T')
            if SH_Override1 > 0:
                draw_rectangle(hwnd, 175, H - 142, 215, H - 156, [127, 127, 247])
                SH_Override1 -= 1
            else:
                draw_rectangle(hwnd, 175, H - 142, 215, H - 156, [247, 247, 247])
            draw_text(hwnd, 195, H - 144, 12, [15, 15, 15], "Play", 'M', 'B')
            if SH_Override0 > 0:
                draw_rectangle(hwnd, 225, H - 142, 265, H - 156, [127, 127, 247])
                SH_Override0 -= 1
            else:
                draw_rectangle(hwnd, 225, H - 142, 265, H - 156, [247, 247, 247])
            draw_text(hwnd, 245, H - 144, 12, [15, 15, 15], "Pause", 'M', 'B')
            draw_rectangle(hwnd, 128, H - 138, 162, H - 122, ([127, 127, 247] if Recording_Speed == 1 else [247, 247, 247]))
            draw_text(hwnd, 145, H - 125, 12, [15, 15, 15], "1.0x", 'M', 'B')
            draw_rectangle(hwnd, 171, H - 138, 205, H - 122, ([127, 127, 247] if Recording_Speed == 2 else [247, 247, 247]))
            draw_text(hwnd, 106 + 85 - 3, H - 125, 12, [15, 15, 15], "0.5x", 'M', 'B')
            draw_rectangle(hwnd, 214, H - 138, 248, H - 122, ([127, 127, 247] if Recording_Speed == 5 else [247, 247, 247]))
            draw_text(hwnd, 231, H - 125, 12, [15, 15, 15], "0.2x", 'M', 'B')
            draw_rectangle(hwnd, 257, H - 138, 291, H - 122, ([127, 127, 247] if Recording_Speed == 10 else [247, 247, 247]))
            draw_text(hwnd, 274, H - 125, 12, [15, 15, 15], "0.1x", 'M', 'B')
            draw_rectangle(hwnd, 35, H - 103, 65, H - 87, ([127, 127, 247] if ProtectionLevel == 0 else [247, 247, 247]))
            draw_text(hwnd, 50, H - 90, 12, [15, 15, 15], "0", 'M', 'B')
            draw_rectangle(hwnd, 75, H - 103, 105, H - 87, ([127, 127, 247] if ProtectionLevel == 1 else [247, 247, 247]))
            draw_text(hwnd, 90, H - 90, 12, [15, 15, 15], "1", 'M', 'B')
            draw_rectangle(hwnd, 115, H - 103, 145, H - 87, ([127, 127, 247] if ProtectionLevel == 2 else [247, 247, 247]))
            draw_text(hwnd, 130, H - 90, 12, [15, 15, 15], "2", 'M', 'B')
            draw_rectangle(hwnd, 155, H - 103, 185, H - 87, ([127, 127, 247] if ProtectionLevel == 3 else [247, 247, 247]))
            draw_text(hwnd, 170, H - 90, 12, [15, 15, 15], "3", 'M', 'B')
            draw_rectangle(hwnd, 195, H - 103, 225, H - 87, ([127, 127, 247] if ProtectionLevel == 4 else [247, 247, 247]))
            draw_text(hwnd, 210, H - 90, 12, [15, 15, 15], "4", 'M', 'B')
            draw_rectangle(hwnd, 235, H - 103, 265, H - 87, ([127, 127, 247] if ProtectionLevel == 5 else [247, 247, 247]))
            draw_text(hwnd, 250, H - 90, 12, [15, 15, 15], "5", 'M', 'B')
            draw_rectangle(hwnd, 130, H - 83, 166, H - 67, ([127, 247, 127] if Show_Watermark else [247, 247, 247]))
            draw_text(hwnd, 148, H - 70, 12, [15, 15, 15], "YES", 'M', 'B')
            draw_rectangle(hwnd, 171, H - 83, 207, H - 67, ([247, 247, 247] if Show_Watermark else [247, 127, 127]))
            draw_text(hwnd, 189, H - 70, 12, [15, 15, 15], "NO", 'M', 'B')
            draw_rectangle(hwnd, 120, H - 63, 156, H - 47, ([127, 247, 127] if Show_Statistics else [247, 247, 247]))
            draw_text(hwnd, 138, H - 50, 12, [15, 15, 15], "YES", 'M', 'B')
            draw_rectangle(hwnd, 161, H - 63, 197, H - 47, ([247, 247, 247] if Show_Statistics else [247, 127, 127]))
            draw_text(hwnd, 179, H - 50, 12, [15, 15, 15], "NO", 'M', 'B')
            draw_rectangle(hwnd, 35, H - 42, 85, H - 25, [247, 247, 247])
            draw_text(hwnd, 60, H - 28, 12, [15, 15, 15], "Hide", 'M', 'B')
            draw_rectangle(hwnd, 95, H - 42, 145, H - 25, ([127,127,247] if Toolbox_Open else [247, 247, 247]))
            draw_text(hwnd, 120, H - 28, 12, [15, 15, 15], "Toolbox", 'M', 'B')
            draw_rectangle(hwnd, 155, H - 42, 205, H - 25, [247, 127, 127])
            draw_text(hwnd, 180, H - 28, 12, [15, 15, 15], "Unload", 'M', 'B')
            if Timer_Reset > 0:
                draw_rectangle(hwnd, 215, H - 42, 285, H - 25, [127, 127, 247])
                Timer_Reset -= 1
            else:
                draw_rectangle(hwnd, 215, H - 42, 285, H - 25, [247, 247, 247])
            draw_text(hwnd, 250, H - 28, 12, [15, 15, 15], "Reset timer", 'M', 'B')
            if Play_Reset > 0:
                draw_rectangle(hwnd, 215, H - 63, 285, H - 47, [127, 127, 247])
                Play_Reset -= 1
            else:
                draw_rectangle(hwnd, 215, H - 63, 285, H - 47, [247, 247, 247])
            draw_text(hwnd, 250, H - 50, 12, [15, 15, 15], "Play + Reset", 'M', 'B')
        else:
            draw_rectangle(hwnd, 0, H - 23, 50, H - 6, [255, 191, 0])
            draw_text(hwnd, 25, H - 8, 12, [15, 15, 15], "Show", 'M', 'B')
        if Show_Statistics:
            draw_rectangle(hwnd, W - 265, H - 250, W - 15, H - 15, [63, 63, 63])
            draw_text(hwnd, W - 250, H - 240, 20, [191, 191, 255], "Statistics: ", 'L', 'T')
            draw_text(hwnd, W - 250, H - 200, 12, [191, 191, 191], "Song: ", 'L', 'T')
            draw_text(hwnd, W - 30, H - 200, 12, [191, 191, 191], File[0:22].strip() + '...' if File.__len__() > 25 else File.strip(), 'R', 'T')
            draw_text(hwnd, W - 250, H - 180, 12, [191, 191, 191], "Time lapsed: ", 'L', 'T')
            draw_text(hwnd, W - 30, H - 180, 12, [191, 191, 191], Format_Time(time.time() - start_time), 'R', 'T')
            draw_text(hwnd, W - 250, H - 160, 12, [191, 191, 191], "Dialogue size: ", 'L', 'T')
            draw_text(hwnd, W - 30, H - 160, 12, [191, 191, 191], "(" + str(W) + "," + str(H) + ")", 'R', 'T')
            draw_text(hwnd, W - 250, H - 140, 12, [191, 191, 191], "Cursor position: ", 'L', 'T')
            draw_text(hwnd, W - 30, H - 140, 12, [191, 191, 191], "(" + str(CX) + "," + str(CY) + ")", 'R', 'T')
            draw_text(hwnd, W - 250, H - 120, 12, [191, 191, 191], "Page flip: ", 'L', 'T')
            draw_text(hwnd, W - 30, H - 120, 12, ([0, 191, 0] if Prev_Flipping else [191, 0, 0]), str(Prev_Flipping), 'R', 'T')
            draw_text(hwnd, W - 250, H - 100, 12, [191, 191, 191], "Brightness of the darkest: ", 'L', 'T')
            draw_text(hwnd, W - 30, H - 100, 12, [191, 191, 191], str(Brightness), 'R', 'T')
            draw_text(hwnd, W - 250, H - 80, 12, [191, 191, 191], "Misrecognition protector state: ", 'L', 'T')
            draw_text(hwnd, W - 30, H - 80, 12, [191, 191, 191], str(MisrecognitionProtect), 'R', 'T')
            draw_text(hwnd, W - 250, H - 60, 12, [191, 191, 191], "App Version: ", 'L', 'T')
            draw_text(hwnd, W - 30, H - 60, 12, [191, 191, 191], Version, 'R', 'T')
            draw_text(hwnd, W - 250, H - 40, 12, [191, 191, 191], "Compatibility: ", 'L', 'T')
            if Version.__contains__("Professional") or Version.__contains__("Standard"):
                draw_text(hwnd, W - 30, H - 40, 12, [0, 191, 0], "Fully Compatible", 'R', 'T')
            elif Version.__contains__("Lite"):
                draw_text(hwnd, W - 30, H - 40, 12, [191, 0, 0], "Incompatible", 'R', 'T')
            else:
...
#Search for main window
    ...
    if depth == 0:
        ...
        if window_class == 'ScrollBar' or window_class == 'Button' or str(window_class).split(':')[0:-2] != ScoreMainClass[0:-2]:
            ...
            print("\033[34mScore editing window handle found! ")
            if not UI_Debug_Mode:
                print("\033[95m\nGreat! You've correctly loaded your MIDI file in Singer Song Writer and opened the score editing window. ")
                print("There are a few more things we have to do before we can record your video. ")
                print("First of all, do you have cheat engine installed on your device? ")
                print("If yes, press 'Z' to continue. Otherwise, press 'X' to install. ")
                playsound.playsound(".\\Resources\\KDE_Click.wav")
                while not keyboard.is_pressed('z') and not keyboard.is_pressed('x'):
                    pass
                ...
                print("Choose \"BitBlt\" as the capture method")
                ...
                print("Please click the 'yes' button when prompted. ")
                print("Note: If you are using versions of Singer Song Writer other than 10 Professional, do not forget to correct the process name in the cheat table. ")
                playsound.playsound(".\\Resources\\KDE_Click.wav")
            ...
            print("\033[92m\nCongratulation: \nSSW Recorder Plugin Loaded Successfully! \033[95m")
            print("\033[95m\nIf you did everything correctly, you should now be able to see the SSW Recorder watermark on the right top corner. ")
            print("The watermark can be removed by changing an option in the control center, but it makes me happy if you could credit my program in your video. ")
            ...
            while True:
                ...
                if mouse.is_pressed(mouse.LEFT) and (not Prev_MouseClick):
                    Process_Mouse_Input(hwnd, Mouse[0] - ToScreen[0], Mouse[1] - ToScreen[1], ClintRECT[2], ClintRECT[3])
                    Prev_MouseClick = True
                time.sleep(1 / 128)
                Main_Program(hwnd, ClintRECT[2], ClintRECT[3], Mouse[0] - ToScreen[0], Mouse[1] - ToScreen[1])
...
def Converter():
    ...
    def ToCh1(track, count):
        ...
        for event in track:
            now += event.time
            if event.type in ['note_on', 'note_off', 'aftertouch', 'polytouch']:
                event.channel = 0
            elif event.type in ['control_change', 'program_change', 'pitchwheel']:
                event.channel = 1
...
if keyboard.is_pressed('shift'):
    ...
    print("You should not be here usually, please restart this app while making sure your 'Shift' key is not being pressed while the app is launching. ")
...
while True:
    ...
    if Maximum_Depth == 0:
        ...
        print("It looks like Singer Song Writer is not running at the moment, please run the program first before using this plugin. ")
        print("If you have not yet installed Singer Song Writer, press 'X' to install the program. ")
        ...
        print("\033[95m\nTroubleshoot assistant: ")
        print("It looks like you've already opened Singer Song Writer but haven't yet load any MIDIs for it to play. ")
        print("Also for better appearance, we strongly suggest you merge all channels and all tracks in your midi into one. ")
        print("This can be done easily through a small tool I wrote in python, press 'M' to open it. ")
        print("If you are having problems reading Japanese and don't know which button is \"Open\", press 'X' on your keyboard and we will open the dialog for you. ")
        print("By the way, don't forget to uncheck 'MIDファイル読み込み時に楽譜の調整を行う' on the bottom of the dialog for better results. ")
        print("Press 'Z' to try searching for the handle again. ")
        playsound.playsound(".\\Resources\\KDE_Error.wav")
        while not keyboard.is_pressed('z') and not keyboard.is_pressed('x') and not keyboard.is_pressed('m'):
            time.sleep(1 / 128)
        threading.Thread(target=playsound.playsound, args=(".\\Resources\\KDE_Click.wav",)).start()
        if keyboard.is_pressed('x'):
            ...
            Converter()
        while keyboard.is_pressed('z') or keyboard.is_pressed('x') or keyboard.is_pressed('c'):
            time.sleep(1 / 128)
    elif Maximum_Depth == 2:
        print("\033[91m\nCRITICAL ERROR: \nFailed to obtain the handle of the score editing window in Singer Song Writer! ")
        print("\033[95m\nTroubleshoot assistant: ")
        print("It looks like you've already loaded a MIDI in Singer Song Writer but haven't yet opened the score editing window inside. ")
        print("If you are having problems reading Japanese and don't know which button is \"Open Score Editing Window\", press 'X' on your keyboard and we will open it for you. ")
        print("Also, for better appearance, we strongly suggest you merge all channels and all tracks in your midi into one. ")
        print("This can be done easily through a small tool I wrote in python, press 'M' to open it. ")
        print("Press 'Z' to try searching for the handle again. \n")
        playsound.playsound(".\\Resources\\KDE_Error.wav")
        ...
        if keyboard.is_pressed('x'):
            ...
            Converter()
        while keyboard.is_pressed('z') or keyboard.is_pressed('x') or keyboard.is_pressed('m'):
            time.sleep(1 / 128)